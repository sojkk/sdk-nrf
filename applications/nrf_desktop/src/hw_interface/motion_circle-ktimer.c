...
static int leds_init(void)
{
	...
	int err = gpio_pin_configure(led_port, DT_ALIAS_LED2_GPIOS_PIN, GPIO_DIR_OUT);
	...
	if(led_port != NULL) {
		(void)gpio_pin_write(led_port, DT_ALIAS_LED2_GPIOS_PIN, LED_OFF); 
	}
...
static bool handle_button_event(const struct button_event *event)
{
	...
	if(dir == DIR_START)
	{
		...
		/* Lit LED3 */
		if(led_port != NULL) {
			(void)gpio_pin_write(led_port, DT_ALIAS_LED2_GPIOS_PIN, LED_ON); 
		}
