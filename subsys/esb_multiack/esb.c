...
static ppi_channel_t ppi_ch_radio_address_timer_capture3; //<--multiack

static uint32_t ppi_all_channels_mask;
static uint32_t ppi_channels_multiack_mode; //<--multiack
...
static void on_radio_end_tx_wait_for_multiack(void);		//<--multiack
static void on_radio_disabled_tx_wait_for_multiack(void);	//<--multiack
...
static bool rx_fifo_push_rfbuf(uint8_t pipe, uint8_t pid)
{
	...
	if(m_config_local.multi_ack) //<--multiack
	{
		m_rx_fifo.p_payload[m_rx_fifo.entry_point]->timestamp = NRF_ESB_SYS_TIMER->CC[3];
	}
...
static void sys_timer_init(void)
{
	...
	ESB_SYS_TIMER->BITMODE = TIMER_BITMODE_BITMODE_32Bit; //<--multiack
...
static void ppi_init(void)
{
	...
	nrfx_dppi_channel_alloc(&ppi_ch_radio_address_timer_capture3);	//<--multiack
	...
	NRF_RADIO->PUBLISH_ADDRESS		  = DPPIC_SUBSCRIBE_CHG_EN_EN_Msk | ppi_ch_radio_address_timer_capture3; //<<--multiack
	ESB_SYS_TIMER->SUBSCRIBE_CAPTURE[3] = DPPIC_SUBSCRIBE_CHG_EN_EN_Msk | ppi_ch_radio_address_timer_capture3; //<<--multiack
	...
	nrfx_ppi_channel_alloc(&ppi_ch_radio_address_timer_capture3);	//<--multiack
	...
	nrfx_ppi_channel_assign(ppi_ch_radio_address_timer_capture3,
		(uint32_t)&NRF_RADIO->EVENTS_ADDRESS, (uint32_t)&NRF_ESB_SYS_TIMER->TASKS_CAPTURE[3]);	//<--multiack
	...
	ppi_channels_multiack_mode(1 << ppi_ch_radio_ready_timer_start) | (1 << ppi_ch_timer_compare1_radio_txen); //<--multiack
}
...
static void on_radio_disabled_tx(void)
{
	...
	if(m_config_local.multi_ack)
    {
        ESB_SYS_TIMER->CC[0] = m_config_local.ptx_ack_duration;
        ESB_SYS_TIMER->CC[1] = m_config_local.ptx_ack_duration + m_config_local.retransmit_delay - 130;
       
		nrfx_gppi_channels_enable( (1 << ppi_ch_radio_ready_timer_start) | 
								   (1 << ppi_ch_timer_compare0_radio_disable) |
								   (1 << ppi_ch_radio_address_timer_capture3)   );
		nrfx_gppi_channels_disable( (1 << ppi_ch_timer_compare1_radio_txen | ( 1 << ppi_ch_radio_address_timer_stop) );

	   //NRF_PPI->CHENSET = (1 << NRF_ESB_PPI_TIMER_START) | (1 << NRF_ESB_PPI_RX_TIMEOUT) | 
       //                    (1 << NRF_ESB_PPI_TIMER_CAPTURE);
       // NRF_PPI->CHENCLR = (1 << NRF_ESB_PPI_TX_START) | (1 << NRF_ESB_PPI_TIMER_STOP);
	   
        NRF_RADIO->SHORTS &= ~RADIO_SHORTS_END_DISABLE_Msk;
        NRF_RADIO->SHORTS |= RADIO_SHORTS_END_START_Msk;
        NRF_RADIO->INTENSET = RADIO_INTENSET_END_Msk;
        on_radio_end = on_radio_end_tx_wait_for_multiack;
        on_radio_disabled = on_radio_disabled_tx_wait_for_multiack;
    }
    else
    {
		...
		nrfx_gppi_channels_enable( (1 << ppi_ch_radio_ready_timer_start) | 
								   (1 << ppi_ch_timer_compare0_radio_disable) |
								   (1 << ppi_ch_radio_address_timer_stop) );
...
static volatile bool multiack_any_ack_received = false;

static void on_radio_end_tx_wait_for_multiack(void)
{
    uint32_t timer_value;
    ESB_SYS_TIMER->TASKS_CAPTURE[3] = 1;
    timer_value = ESB_SYS_TIMER->CC[3];
    // If the radio has received a packet and the CRC status is OK
    if (NRF_RADIO->CRCSTATUS != 0)
    {
        //m_last_tx_attempts = m_config_local.retransmit_count - m_retransmits_remaining + 1;
        multiack_any_ack_received = true;
...
static void on_radio_disabled_tx_wait_for_multiack(void)
{
    // Make sure the timer will not deactivate the radio if a packet is received
	nrfx_gppi_channels_disable( (1 << ppi_ch_radio_ready_timer_start) | 
								(1 << ppi_ch_timer_compare0_radio_disable) |
								(1 << ppi_ch_radio_address_timer_stop) );
    ...
    if(multiack_any_ack_received)
    {
        ...
        multiack_any_ack_received = false;
    }
    else
    {
        // For now, ignore the retransmission feature. With multiack there is no retransmit. 
        ESB_SYS_TIMER->TASKS_SHUTDOWN = 1;
		nrfx_gppi_channels_disable( 1 << ppi_ch_timer_compare1_radio_txen );
        //NRF_PPI->CHENCLR = (1 << NRF_ESB_PPI_TX_START);
...
static void on_radio_disabled_rx(void)
{
	...
	if ((esb_cfg.selective_auto_ack == false && esb_cfg.multiack == false) ||
	    ((rx_payload_buffer[1] & 0x01) == 1)) {
			ack = true;
		}
	
	// In multi ack mode, the PRX should ignore incoming packets without the ACK bit set (these might come from other PRX's)
    if(esb_cfg.multi_ack && !ack) 
    {
        send_rx_event = false;
    }
	...
	if (ack)
		...
		case ESB_PROTOCOL_ESB_DPL:
			...
			// Make sure the ACK bit is disabled on the ACK packet when multi ack mode is used
            if(esb_cfg.multi_ack) m_tx_payload_buffer[1] &= 0xFE; //<--multiack
			break;
		...
		NRF_RADIO->PACKETPTR = (uint32_t)tx_payload_buffer;
		if(esb_cfg.multi_ack) NRF_RADIO->SHORTS &= ~RADIO_SHORTS_DISABLED_RXEN_Msk; //<--multiack
...
int esb_init(const struct esb_config *config)
{
	...
	// Configure the radio for fast startup
    if(esb_cfg.multi_ack)
        NRF_RADIO->MODECNF0 |= RADIO_MODECNF0_RU_Fast << RADIO_MODECNF0_RU_Pos;	//<--multiack
...
uint32_t nrf_esb_read_rx_payload_02(nrf_esb_payload_02_t * p_payload)
{
	if (!esb_initialized) {
		return -EACCES;
	}
    ...
    p_payload->noack  = m_rx_fifo.p_payload[m_rx_fifo.exit_point]->noack;
		...
		if (p_payload->timestamp < 	PERIPH_TIMESTAMP)
		{	
			memcpy(p_payload->data0, m_rx_fifo.p_payload[m_rx_fifo.exit_point]->data, p_payload->length);
    
		}
		else
		{

			memcpy(p_payload->data1, m_rx_fifo.p_payload[m_rx_fifo.exit_point]->data, p_payload->length);
		}
